...
class ModelArguments:
    ...
    mbart_tokenizer_name: Optional[str] = field(
        default="Helsinki-NLP/opus-mt-en-id", metadata={"help": "Pretrained tokenizer name or path if not the same as model_name"}
    )
...
def main():
    ...
    if training_args.resume_from_checkpoint is None:
        model = FlaxCLIPVisionMarianForConditionalGeneration.from_clip_vision_marian_pretrained(
        ...
        model = FlaxCLIPVisionMarianForConditionalGeneration.from_pretrained(training_args.resume_from_checkpoint)
    config = model.config
    ...
    def collate_fn(examples):
        ...
        with tokenizer.as_target_tokenizer():
            inputs = tokenizer(captions, max_length=data_args.max_seq_length, padding="max_length", return_tensors="np", truncation=True)
        
        decoder_input_ids = shift_tokens_right(inputs["input_ids"], config.marian_config.pad_token_id)
    ...
    for epoch in epochs:
        ...
        for step, batch in enumerate(train_loader):
            ...
            if cur_step % training_args.eval_steps == 0 and cur_step > 0:
                ...
                for val, batch in enumerate(eval_loader):
